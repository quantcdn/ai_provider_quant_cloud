...
class AuthService {
  ...
  public function persistTokens(array $token_data): void {
    ...
    $this->writeKey('quant_cloud_oauth_access_token', $token_data['access_token'], 'OAuth2 access token');

    if (!empty($token_data['refresh_token'])) {
      $this->writeKey('quant_cloud_oauth_refresh_token', $token_data['refresh_token'], 'OAuth2 refresh token');
    }
  ...
  protected function writeKey(string $key_id, string $value, string $label): void {
    ...
    $key = $this->entityTypeManager->getStorage('key')->create([
      ...
      'description' => $label . ' for Quant Cloud API',
